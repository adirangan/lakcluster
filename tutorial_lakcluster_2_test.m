% This generates an example and tests tutorial_lakcluster_2.m ;

disp(sprintf(' '));
disp(sprintf(' This function tests tutorial_lakcluster_2.'));
rng(1);
path_base = './dir_tutorial_lakcluster_test/'; if (~exist(path_base,'dir')); mkdir(path_base); end;
prefix_base = 'tutorial_lakcluster_2_test';
...
pause();
disp(sprintf(' '));
disp(sprintf(' Now we set up the input file for the tutorial_lakcluster_2.m code. '));
...
tutorial_lakcluster_2(sprintf('%s',fname_in));
disp(sprintf(' '));
...
disp(sprintf(' tutorial_lakcluster_2 algorithm found some kind of bicluster. But which one?'));
...
disp(sprintf(' '));
disp(sprintf(' We embark on this task here by calling tutorial_lakcluster_2 with the loopS_flag on. '));
...
disp(sprintf(' Specifically, given the output-ordering of tutorial_lakcluster_2, we consider, for each (m,n), '));
...
if ~load_flag; tutorial_lakcluster_2(sprintf('%s',fname_in),1,128,196); end;
...
disp(sprintf(' additional arguments to tutorial_lakcluster_2.m via an expanded input file. '));
