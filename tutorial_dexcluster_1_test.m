% This generates an example and tests tutorial_dexcluster_1.m ;

disp(sprintf(' '));
disp(sprintf(' This function tests tutorial_dexcluster_1.'));
rng(1);
path_base = './dir_tutorial_dexcluster_test/'; if (~exist(path_base,'dir')); mkdir(path_base); end;
prefix_base = 'tutorial_dexcluster_1_test';
...
C0rows = floor(length(d_ij).^(0.5)); C0cols = floor(length(g_ij).^(0.65)); C0 = 2.5*ones(C0rows,1)*randn(1,C0cols); [prA,pc] = tutorial_hcluster(C0); C0 = C0(prA,pc);
...
disp(sprintf(' Within this data we embed a bicluster: '));
disp(sprintf(' bc0 : this bicluster is differentially-expressed. This is what we want to find. '));
disp(sprintf(' '));
disp(sprintf(' Now we plot the data two times, '));
disp(sprintf(' The first time (left), we show the original data-matrix. '));
disp(sprintf(' The second time (right), we rearrange the rows/cols to reveal the embedded bicluster. '));
ncovs = 0;
cov_mat = zeros(npats,ncovs); 
...
subplot(1,2,1); 
Dtmp = Aall_n(d_ij(:),g_ij(:)) ; 
Xtmp = Aall_n(x_ij(:),g_ij(:)) ; 
colormap(cmap_autumn);
imagesc([min(ctopl,max(cbotl,Dtmp)) ; cbot*ones(pgap,ngenes) ; min(ctopl,max(cbotl,Xtmp)) ],[cbot,ctop]);
set(gca,'Xtick',[],'Ytick',[]); title('original data');
subplot(1,2,2); 
Dtmp = Aall_n(d_ij(p_d_ij_r),g_ij(p0_g_ij_r)) ; 
Xtmp = Aall_n(x_ij(:),g_ij(p0_g_ij_r)) ; 
colormap(cmap_autumn);
imagesc([min(ctopl,max(cbotl,Dtmp)) ; cbot*ones(pgap,ngenes) ; min(ctopl,max(cbotl,Xtmp)) ],[cbot,ctop]);
...
pause();
disp(sprintf(' '));
disp(sprintf(' Now we set up the input file for the tutorial_dexcluster_1.m code. '));
...
disp(' Then we set up the covariate indices (which in this case indicate no covariates).');
...
tutorial_dexcluster_1(sprintf('%s',fname_in));
disp(sprintf(' '));
...
cmap_autumn = colormap('autumn'); cmap_autumn(1,:) = [1,1,1]; cmap_autumn(end-1,:) = 0.85*[1,1,1]; cmap_autumn(end,:) = 0.00*[1,1,1]; clen = size(cmap_autumn,1); colormap(cmap_autumn);
cbot=-1.5; ctop=1.5; cvals = linspace(cbot,ctop,clen); cbotl=cvals(2); ctopl = cvals(end-2); cdiff = cvals(end)-cvals(end-1); 
Dtmp = Aall_n(1+rij,1+cij) ; 
Xtmp = Aall_n( x_ij,1+cij) ; 
subplot(1,2,1);
imagesc([min(ctopl,max(cbotl,Dtmp)) ; cbot*ones(pgap,ngenes) ; min(ctopl,max(cbotl,Xtmp)) ],[cbot,ctop]);
set(gca,'Xtick',[],'Ytick',[]); title('retained rows/cols (upper left) organized via output of dexcluster');
pgap = 32;ggap = 16;
Dtmp = Aall_n(1+rij(1:64),1+cij(1:196)) ; 
subplot(1,2,2);
imagesc([min(ctopl,max(cbotl,Dtmp))],[cbot,ctop]);
...
pause();
disp(sprintf(' '));
disp(sprintf(' We can clearly see some sort of differentially-expressed bicluster in the upper left corner, '));
disp(sprintf(' implying that the tutorial_dexcluster_1 algorithm found something. '));
disp(sprintf(' In order to show that bicluster 0 was retained longest, we plot, as a function of iteration, the '));
disp(sprintf(' overlap fraction of the retained rows (solid) and columns (dashed) with the bicluster: '));
...
disp(sprintf(' overlap with bicluster 0 (red). '));
...
legend('bc0 rows','bc1 cols','Location','SouthEast');
title('fraction of bicluster retained (row=solid, column=dashed)');
...
disp(sprintf(' of the embedded bicluster. In order to do this we examine the iterations taken by the algorithm, '));
...
disp(sprintf(' If these average scores are high then the algorithm is focusing on a bicluster, '));
disp(sprintf(' but if they are low then the algorithm has not yet found anything. '));
disp(sprintf(' '));
...
disp(sprintf(' sixth number: the number of covariate-categories remaining across the patients (not used here). '));
disp(sprintf(' '));
...
disp(sprintf(' '));
disp(sprintf(' We embark on this task here by calling tutorial_dexcluster_1 with the loopS_flag on. '));
...
disp(sprintf(' Specifically, given the output-ordering of tutorial_dexcluster_1, we consider, for each (m,n), '));
...
disp(sprintf(' '));
tutorial_dexcluster_1(sprintf('%s',fname_in),1,128,196);
...
disp(sprintf(' Note that the average scores are quite high for corner submatrices that are around 25-by-75. '));
disp(sprintf(' This protruding shape in the LR2 and LC2 arrays is indicative of revealed structure. '));

figure; cla;
subplot(1,4,[1,2]); imagesc(LR2_ra,[0,1]); colorbar; title('average row score'); set(gca,'Ytick',[1,25,75,100],'Yticklabel',[1,25,75,100],'Xtick',[1,50,75,100],'Xticklabel',[1,50,75,100]); xlabel('columns n'); ylabel('rows m');
subplot(1,4,[3,4]); imagesc(LC2_ra,[0,1]); colorbar; title('average col score'); set(gca,'Ytick',[1,25,75,100],'Yticklabel',[1,25,75,100],'Xtick',[1,50,75,100],'Xticklabel',[1,50,75,100]); xlabel('columns n'); ylabel('rows m');

disp(sprintf(' '));
disp(sprintf(' Paused (push a key). '));
pause();
disp(sprintf(' '));
disp(sprintf(' If we wanted to, we could threshold these average-scores and extract something like this 25-by-75 '));
...
[output,ik_out,ij_out] = tutorial_loopS_threshold(LR2_ra,ones(size(LR2_ra)),rthr,ar_min,ar_max,1);
disp(sprintf(' '));
...
disp(sprintf(' This maximal-area corner is 26-by-75. '));
disp(sprintf(' Note that the colorbar for this plot (LR2_min to LR2_max) is different than the previous figure (0,1). '));
disp(sprintf(' '));
disp(sprintf(' Paused (push a key). '));
pause();
disp(sprintf(' '));
disp(sprintf(' At this point we can extract this 26-by-75 submatrix and plot it in relation to the controls: '));
...
disp(sprintf(' additional arguments to tutorial_dexcluster_1.m via an expanded input file. '));
...
cmap_autumn = colormap('autumn'); cmap_autumn(1,:) = [1,1,1]; cmap_autumn(end-1,:) = 0.85*[1,1,1]; cmap_autumn(end,:) = 0.00*[1,1,1]; clen = size(cmap_autumn,1); colormap(cmap_autumn);
cbot=-1.5; ctop=1.5; cvals = linspace(cbot,ctop,clen); cbotl=cvals(2); ctopl = cvals(end-2); cdiff = cvals(end)-cvals(end-1); 
Dtmp = Aall_n(1+rij_sub,1+cij_sub) ; 
Xtmp = Aall_n( x_ij,1+cij_sub) ; 
imagesc([ min(ctopl,max(cbotl,Dtmp)) ; cbot*ones(pgap,length(cij_sub)) ;  min(ctopl,max(cbotl,Xtmp)) ],[cbot,ctop]);
