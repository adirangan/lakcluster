function [tr_base,tr_ra_,tc_base,tc_ra_,cij_base,cij_ra_] = tutorial_collate(path_base,gen_fname,frwd_vs_back);
% designed to collect and plot results generated by using tutorial_w1.m (including multiple biclusters) : ;
% loads trace and -- if load_xdrop_flag is set to 1 -- also loads xdrop ;
...
if (exist(tmp_xdrop_fname,'file') & exist(tmp_trace_fname,'file')); ox_ra(1+no)=1; end;
end;% for no=0:Omax-1; 
O_ra = find(ox_ra);
NO = length(O_ra);
disp(sprintf(' %% found %d biclusters',NO));

clear tr_base_ra tc_base_ra nc_base_ra;
path_use = path_base;
prefix = prefix_base; 
for no=1:NO;
no_flag = O_ra(no)-1;
...
if (exist(sprintf('%s%s_%s_out_trace.txt',path_use,prefix,frwd_vs_back),'file') & exist(sprintf('%s%s_%s_out_xdrop.txt',path_use,prefix,frwd_vs_back),'file')); ex_ra_(np)=1; end; 
end;% for np=1:Pmax; 
...
cij_ra_ = zeros(size(cij_base_ra,1),NP_xxxx_);
for (np=1:NP_xxxx_);
...
tc_ra_(:,np) = t_tmp(:,5);
nc_ra_(:,np) = t_tmp(:,6);
%disp(sprintf('%s%s_%s_out_xdrop.txt',path_use,prefix,frwd_vs_back));
out_xdrop = textread(sprintf('%s%s_%s_out_xdrop.txt',path_use,prefix,frwd_vs_back));
cdrop = out_xdrop(:,2); cijp = cdrop(find(cdrop>=0)); cijp = [cijp(:) ; setdiff(g_ij(:)-1,cijp(:))]; cijp = cijp(end:-1:1);
[tmp,tmp,tmp_ij] = intersect(g_ij,1+cijp,'stable');
cij_ra_(:,np) = tmp_ij;
...
% One subplot per permutation_type ;
zmx1 = 2;
zmx2 = 4;
...
%{
% Here we plot the histogram of average rank (across permutations) of each gene. ;
% One subplot per permutation_type. ;
plot_flag=1;
if plot_flag;
figure;cla;
subplot(1,1,1);
%hc = hist(cij_ra_,linspace(1,size(cij_base_ra,1),128));
hist(mean(cij_ra_,2),linspace(1,size(cij_base_ra,1),128));
xlim([0,length(g_ij)]); xlabel('rank'); ylabel('#'); title('histogram of average-rank (across permutations) of each gene');
set(gcf,'Position',[100, 100, 1049, 895]);
...
set(gcf,'Position',[100, 100, 1049, 895]);
print('-depsc',sprintf('%s%s_%s_collate_FIGF.eps',path_FIGS,prefix_base,frwd_vs_back));
print('-djpeg',sprintf('%s%s_%s_collate_FIGF.jpg',path_FIGS,prefix_base,frwd_vs_back));
end;%if plot_flag;

path_FIGS = sprintf('%sdir_FIGS/',path_base); if ~exist(path_FIGS,'dir'); mkdir(path_base,'dir_FIGS'); end;
save(sprintf('%s%s_%s_collate.mat',path_FIGS,prefix_base,frwd_vs_back),'Omax','ox_ra','O_ra','NO','Pmax','ex_ra_','path_pre_prm','P_xxxx_ra_','NP_xxxx_','tr_base_ra','tr_ra_','tc_base_ra','tc_ra_','nc_base_ra','nc_ra_','cij_base_ra','cij_ra_','tr_avg_pval_','tr_zmx_pval_','tr_max_pval_','tr_avg_','tr_zmx_','tr_max_','tr_base_avg_ra','tr_base_zmx_ra','tr_base_max_ra','tr_cmb_pval_');
